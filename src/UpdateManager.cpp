...
void UpdateManager::onVersionCheckFinished(QNetworkReply* reply) {
    ...
    if (latestNum.isNull() || currentNum.isNull()) {
        LOG_INFO(QString("Could not parse version numbers, falling back to string compare.").arg(latestVersion).arg(m_currentVersion));
        if (latestVersion <= m_currentVersion) {
            LOG_INFO(QString("Current version is up to date:").arg(m_currentVersion));
            return;
        }
    }
